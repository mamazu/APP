...
public class GUI extends JFrame implements /*WindowListener, ActionListener,*/ Requestable {
    ...
    private static Image stoneRed, stoneBlue, movesUpL, movesDownL, killStone;
    private static boolean imagesLoaded = false;
    private static Font font;
    private static final Color DARKRED = new Color(139, 0, 0), NAVY = new Color(0, 0, 128);
    ...
    private synchronized void loadImages() {
        imagesLoaded = true;
        URL surrenderFile = SplinterTheOmniscientRat.get("surrendercat_icon.png");
        if(surrenderFile != null)
            iconSurrender = new ImageIcon(surrenderFile);
        else
            iconSurrender = new ImageIcon();
        //try{
        ...
        Image stoneRedL = getOvalImage(Color.YELLOW, "red_hl.png");
        stoneBlue = getOvalImage(Color.BLUE, "blue.png");
        Image stoneBlueL = getOvalImage(Color.CYAN, "blue_hl.png");
        if (Setting.RED == Board.MOVESUP) {
            movesUpL = stoneRedL;
            movesDownL = stoneBlueL;
        } else {
            movesDownL = stoneRedL;
            movesUpL = stoneBlueL;
        }
    ...
    private Image getOvalImage(Color cl, String name) {
        try {
            URL file = SplinterTheOmniscientRat.get(name);
            if (file != null) return ImageIO.read(file);
            return null;
        } catch (IOException e) {
    ...
    private class GUIPanel extends JPanel {
        ...
        public GUIPanel(GUI par) {
            ...
            closeButton.addActionListener(e -> parent.dispatchEvent(new WindowEvent(parent, WindowEvent.WINDOW_CLOSING)));
         ...
         * Draws an arrow
         *
         * @param g1    {@link Graphics} where to draw it
         * @param x1    Starting x value
         * @param y1    Starting y value
         * @param angle angle of the line
         * @param len   length of the line
         */
        private void drawArrow(Graphics g1, int x1, int y1, double angle, int len) {
